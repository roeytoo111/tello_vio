...
private:
  ...
  std::thread trajectory_viz_thread_;
  std::condition_variable& cv_run_;
  void trajectoryVisualizationThread();
  void keyboardToCommand(int key);
  void updateRCFromKeyboard();
  
  // Keyboard state tracking
  int rc_left_right_ = 0;      // Roll: left/right (-100 to 100)
  int rc_forward_back_ = 0;    // Pitch: forward/back (-100 to 100)
  int rc_up_down_ = 0;         // Throttle: up/down (-100 to 100)
  int rc_yaw_ = 0;              // Yaw: rotate left/right (-100 to 100)
  std::mutex keyboard_rc_mutex_;
  std::chrono::steady_clock::time_point last_key_press_time_;  // Track last key press
